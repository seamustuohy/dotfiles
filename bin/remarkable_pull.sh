...
function download {
  ...
  if [ -d "$3" ]; then
    # Remove single quotes around file name
    new_file_name=${file_name#"'"}
    new_file_name=${new_file_name%"'"}
    # Replace double extensions with single (i.e. XXX.pdf.pdf)
    extension="${new_file_name##*.}"
    new_file_name="${new_file_name/%.$extension.$extension/.${extension}}"

    annotated_extension="_ANNOTATED.${extension}"
    new_file_name="${new_file_name%.pdf}${annotated_extension}"
